...
function initTechStackVisualization() {
    ...
    try {
        ...
        function animate() {
            ...
            // Store animation ID on the container for cleanup
        ...
        container.innerHTML = `
            <div class="flex flex-wrap justify-center gap-4 p-4">
                ${['Node.js', 'Express', 'MongoDB', 'Supabase', 'HTML5', 'CSS3', 'JavaScript', 'Tailwind'].map(tech =>
...
function initDataFlowVisualization() {
    ...
    try {
        ...
        const connections = [
            { from: 0, to: 1 }, // Client to API
            { from: 1, to: 2 }, // API to Auth
            { from: 1, to: 3 }, // API to Database
            { from: 1, to: 4 }, // API to Storage
            { from: 2, to: 3 }  // Auth to Database
        ];
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            // Set initial position at the 'from' node
        ...
        function animate() {
            ...
            particles.forEach(particle => {
                ...
                if (particle.progress >= 1) {
                    ...
                    // Randomly select a new connection
                    const connectionIndex = Math.floor(Math.random() * connections.length);
                    const connection = connections[connectionIndex];
...
function cleanupThreeJS() {
    ...
    // Clean up tech stack visualization
    const techStackContainer = document.querySelector('#tech-stack-container');
    if (techStackContainer) {
        const animationId = techStackContainer.dataset.animationId;
        if (animationId) {
            cancelAnimationFrame(Number(animationId));
        }

        // Remove canvas
        const canvas = techStackContainer.querySelector('canvas');
        if (canvas) {
            techStackContainer.removeChild(canvas);
        }
    }
    ...
    if (dataFlowContainer) {
        const animationId = dataFlowContainer.dataset.animationId;
        if (animationId) {
            cancelAnimationFrame(Number(animationId));
        }

        // Remove canvas
        const canvas = dataFlowContainer.querySelector('canvas');
        if (canvas) {
            dataFlowContainer.removeChild(canvas);
        }
    }

    // Remove resize handlers
    if (techStackContainer && techStackContainer.dataset.resizeHandler) {
        window.removeEventListener('resize', techStackContainer.dataset.resizeHandler);
    }

    if (dataFlowContainer && dataFlowContainer.dataset.resizeHandler) {
        window.removeEventListener('resize', dataFlowContainer.dataset.resizeHandler);
    }
}
